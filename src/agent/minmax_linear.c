...
#ifdef AGENT_TRAINING
    return run_ea_training_loop(argc, argv, minmax_linear_factory, "models/minmax_linear/best.txt");
#else
    // 1. Create the evaluator
    // Try to load the trained model if it exists, otherwise fallback to defaults
    Evaluator* eval = evaluator_create_minmax_linear("models/minmax_linear/best.txt");
    if (!eval) {
